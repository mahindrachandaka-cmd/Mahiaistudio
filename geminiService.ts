 ...
 * Uses process.env.API_KEY which is injected by the build tool (Vite)
 * from the hosting provider's environment variables.
 */
const createAIInstance = () => {
  const apiKey = process.env.API_KEY;
  if (!apiKey) {
    throw new Error("Gemini API Key is missing. Please set the API_KEY environment variable in your hosting provider's dashboard (e.g., Netlify or Vercel).");
  }
...
async function callGemini(action: (ai: GoogleGenAI) => Promise<any>) {
  try {
    ...
    console.error("Mahi Studio API Error:", error);
    
    if (error.message?.includes("API key not valid") || error.message?.includes("403") || error.message?.includes("401")) {
      throw new Error("Invalid API Key. Please verify your key in the Google AI Studio dashboard and update your hosting environment variables.");
    }
